...
with open('time_shift_summary.json', encoding='utf-8') as f:
    summary = json.load(f)

report = f"""# TIME_SHIFT_SENSITIVITY_REPORT.md
...
- **Total held-out positive test clips evaluated**: {summary['test_counts']['total']}
- **Real Human Recordings**: {summary['test_counts']['real']}
- **TTS (Piper) Recordings**: {summary['test_counts']['tts']}
...
- **Mean Speech Onset**: {summary['train_stats']['mean_onset_ms']:.1f} ms (Median: {summary['train_stats']['median_onset_ms']:.1f} ms)
- **Mean Speech Center**: {summary['train_stats']['mean_center_ms']:.1f} ms (Median: {summary['train_stats']['median_center_ms']:.1f} ms)
...
for off in [0, 100, 200, 300, 400, 500, 600]:
    if str(off) in summary['offset_tpr_0_50']:
        d = summary['offset_tpr_0_50'][str(off)]
        report += f"| {off} | {d['n']} | **{d['tpr']*100:.2f}%** | {d['tpr_real']*100:.2f}% | {d['tpr_tts']*100:.2f}% | {d['mean']:.4f} | {d['median']:.4f} | {d['p10']:.4f} |\n"
...
with open('TIME_SHIFT_SENSITIVITY_REPORT.md', 'w', encoding='utf-8') as f:
    f.write(report)
