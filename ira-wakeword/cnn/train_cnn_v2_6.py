...
train_cnn_v2_6.py
=================
IRA CNN V2.6 -- Controlled Positive Augmentation SNR Experiment

ONLY change from V2.3:
  Speech-background target SNR in positive mixed examples is constrained
  uniformly to [+10 dB, +25 dB], so the active wakeword is clearly louder.
  Ambient-background positive examples use the original V2.3 SNR distribution.
  Positive mixed composition restored to V2.3's 50% ambient / 50% speech.
...
Validation EXCLUSIVELY loads expanded_val_v2_3_frozen.npz to ensure strict
comparability with historic models.
"""
...
BEST_LOSS_PATH = MODEL_OUT_DIR / "ira_cnn_v2_6_best_loss.keras"
BEST_SFPR_PATH = MODEL_OUT_DIR / "ira_cnn_v2_6_best_speech_fpr.keras"
FINAL_PATH     = MODEL_OUT_DIR / "ira_cnn_v2_6_final.keras"

HISTORY_CSV        = REPO_ROOT / "v2_6_training_history.csv"
STATS_JSON         = REPO_ROOT / "v2_6_augmentation_stats.json"
SAMPLING_STATS_JSON= REPO_ROOT / "v2_6_sampling_stats.json"
...
# V2.6 Speech SNR Intervention
SPEECH_SNR_RANGE = (10.0, 25.0)

# V2.6 batch recipe (unchanged)
N_EASY_SP   = 14
N_MED_SP    =  7
N_HARD_SP   =  4
N_AMBIENT   =  5
...
def augment_positive(path: str, amb_paths: list, sp_paths: list, stats: dict) -> np.ndarray:
    ...
    # V2.6: Restore 50/50 probability, identical to V2.3
    if sp_paths and (random.random() < 0.5 or not amb_paths):
        bg_seg, bg_src, _ = get_random_bg_segment(sp_paths, stats)
        stats["speech"] += 1
        
        # V2.6 Intervention: Speech SNR uniformly +10 to +25 dB
        snr_db = random.uniform(SPEECH_SNR_RANGE[0], SPEECH_SNR_RANGE[1])
        stats["speech_snr_vals"].append(snr_db)
    else:
        bg_seg, bg_src, _ = get_random_bg_segment(amb_paths, stats)
        stats["ambient"] += 1
        
        # Ambient SNR unchanged from V2.3
        ...
        stats["ambient_snr_vals"].append(snr_db)
...
# Generator
# ===========================================================================
class V26DataGenerator(tf.keras.utils.PyDataset):
...
def main():
    ...
    print("\nBuilding V2.6 difficulty pools (TRAIN speech only)...")
    ...
    # -------------------------------------------------------------------------
    print("\n" + "="*65)
    print("PRE-TRAINING SANITY CHECK (V2.6)")
    print("="*65)
    ...
    gen = V26DataGenerator(train_pos, easy_sp, med_sp, hard_sp, train_amb, train_oth,
                           train_amb, all_train_sp, train_stats)

    print("  Running 20-batch sanity pass to verify positive augmentations...")
    for i in range(20): gen[i]
    
    print(f"  Positive clean count       : {train_stats['clean']}")
    print(f"  Positive ambient-mixed count: {train_stats['ambient']}")
    print(f"  Positive speech-mixed count : {train_stats['speech']}")
    ...
    print(f"  Mixed composition approx   : {amb_pct:.1f}% ambient / {sp_pct:.1f}% speech")
    
    amb_snr_vals = train_stats["ambient_snr_vals"]
    sp_snr_vals = train_stats["speech_snr_vals"]
    
    if amb_snr_vals:
        print(f"  Ambient-mixed target SNR   : min {min(amb_snr_vals):.2f} / mean {np.mean(amb_snr_vals):.2f} / max {max(amb_snr_vals):.2f}")
    if sp_snr_vals:
        print(f"  Speech-mixed target SNR    : min {min(sp_snr_vals):.2f} / mean {np.mean(sp_snr_vals):.2f} / max {max(sp_snr_vals):.2f}")
    
    assert 40 <= sp_pct <= 60, "Mixed background composition not approximately 50/50"
    if sp_snr_vals:
        assert min(sp_snr_vals) >= SPEECH_SNR_RANGE[0] - 1e-4, "Speech SNR went below +10 dB!"
        assert max(sp_snr_vals) <= SPEECH_SNR_RANGE[1] + 1e-4, "Speech SNR went above +25 dB!"
    
    # Reset before real training
    for k in train_stats: 
        if isinstance(train_stats[k], list): train_stats[k] = []
        else: train_stats[k] = 0
    ...
    print("\nV2.6 TRAINING PIPELINE VALIDATION: PASS")

    # Load frozen validation arrays
    print("\nLoading EXACT frozen validation NPZ...")
    if not FROZEN_VAL_NPZ.exists():
        raise RuntimeError("expanded_val_v2_3_frozen.npz not found! Must freeze first.")
        
    val_data = np.load(str(FROZEN_VAL_NPZ), allow_pickle=True)
    val_X_all = val_data["X"]
    val_labels_all = val_data["labels"]
    val_neg_cats = val_data["negative_categories"]
    
    val_X = val_X_all
    val_y = val_labels_all
    
    # For callbacks we need pos, sp, amb separated
    pos_idx = (val_labels_all == 1.0).flatten()
    sp_idx = (val_neg_cats == "speech").flatten()
    amb_idx = (val_neg_cats == "ambient").flatten()
    
    val_pos_X = val_X_all[pos_idx]
    val_sp_X = val_X_all[sp_idx]
    val_amb_X = val_X_all[amb_idx]
    
    print(f"  Loaded frozen arrays: pos={len(val_pos_X)} sp={len(val_sp_X)} amb={len(val_amb_X)}")
    
    # -------------------------------------------------------------------------
    # TRAINING
    # -------------------------------------------------------------------------
    print("\n" + "="*65)
    print("TRAINING IRA CNN V2.6")
    print("="*65)
    ...
    if BEST_SFPR_PATH.exists():
        ...
        go_target_sfpr = threshold_sweep(m_sfpr, val_pos_X, val_sp_X, val_amb_X, "V2.6 best_speech_fpr")
    else:
    ...
    if BEST_LOSS_PATH.exists():
        ...
        go_target_bl = threshold_sweep(m_bl, val_pos_X, val_sp_X, val_amb_X, "V2.6 best_loss")
    else:
        print("  best_loss model not found.")

    # -------------------------------------------------------------------------
    # COMPARE V2.3 vs V2.6
    # -------------------------------------------------------------------------
    print("\n" + "="*65)
    print("V2.3 vs V2.6 COMPARISON (FROZEN EXPANDED_VAL)")
    print("="*65)
    
    compare_models = {
        "V2.3 best_loss": MODEL_OUT_DIR / "ira_cnn_v2_3_best_loss.keras",
        "V2.3 best_sfpr": MODEL_OUT_DIR / "ira_cnn_v2_3_best_speech_fpr.keras",
        "V2.6 best_loss": BEST_LOSS_PATH,
        "V2.6 best_sfpr": BEST_SFPR_PATH,
    }
    ...
    for name, mp in compare_models.items():
        ...
        # Find best thr: lowest Speech FPR maintaining TPR >= 95.0
        best_thr = None
        best_sfpr = float('inf')
        ...
        if best_thr is not None:
            ...
            print(f"{name:<20}  FAILED to maintain TPR >= 95.0")

    print("\n" + "="*65)
    print("GO / NO-GO")
    print("="*65)
    if go_target_sfpr is not None or go_target_bl is not None:
        print("  OVERALL V2.6: GO")
    else:
        print("  OVERALL V2.6: NO-GO")
